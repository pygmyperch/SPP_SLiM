...
setwd("/Users/chrisbrauer/Projects/spp_sim_test/Ne1000/sim_reps/")

scenario_dirs <- list.dirs('.', full.names = T, recursive = F)

NE1000_res <- lapply(scenario_dirs, function(d) {
  ...
  mtext(bquote(Total~population~size == .(format("1000", digits = 1))), adj=0.03, padj=1.5, outer = FALSE)
...
df = do.call(rbind, NE1000_res)
colnames(df) <- c("Fragments", "Generations")
df <- df[order(as.numeric(as.character(df$Fragments))), ]
write.table(df, "Ne1000_results.txt", quote = FALSE, row.names = FALSE)
