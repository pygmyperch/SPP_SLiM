...
setwd("/Users/chrisbrauer/Projects/spp_sim_test/Ne500/sim_reps/")

scenario_dirs <- list.dirs('.', full.names = T, recursive = F)

NE500_res <- lapply(scenario_dirs, function(d) {
  ...
  tab <- as.data.frame(do.call(rbind, res[1:21]))# set number of generations to plot here
  ...
  lines(x, predict(poly2.mod, data.frame(x=x)), col="blue")
  xval <- spline(x = poly2.mod$fitted.values, y = x, xout = 0.2)$y
  points(xval, 0.2, col = "red", lwd = 3, pch = 8)
  mtext(bquote(Total~population~size == .(format("500", digits = 1))), adj=0.03, padj=1.5, outer = FALSE)
...
df = do.call(rbind, NE500_res)
colnames(df) <- c("Fragments", "Generations")
df <- df[order(as.numeric(as.character(df$Fragments))), ]
write.table(df, "Ne500_results.txt", quote = FALSE, row.names = FALSE)
